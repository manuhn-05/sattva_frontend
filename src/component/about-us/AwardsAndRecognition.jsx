        ...
        <section className='grid grid-cols-1 md:grid-cols-3 gap-4'>
          {
            ...
            AWARDS_AND_RECOGNITION?.map((award) => {
              return (
                <div key={award.id} className={`bg-white p-[2%] dark:bg-[#0F172A] rounded-lg shadow hover:shadow-xl duration-75
                shadow-slate-200 dark:shadow-slate-800 overflow-hidden max-w-[350px] `}>
                  <div className='h-[200px]'>
                    <img src={award.imageSrc} alt={award?.heading} className='w-full h-full' />
                  </div>
                  <div className='mt-[7%] w-[90%] mx-auto '>
                    <h4 className='text-[1.2rem] font-bold dark:text-white my-[2%] hover:text-green-600 duration-75'>{award.heading}</h4>
