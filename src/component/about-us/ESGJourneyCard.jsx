...
const ESGJourneyCard = ({ item }) => {
    ...
    return (
        <div className='border-gray-400 border-[0.015em] shadow-lg mx-[3%] bg-white dark:bg-slate-600 p-[3%] rounded-md lg:min-h-[15vw] hover:shadow-2xl hover:scale-[0.95] duration-75'>
            <div>
                <h3 className={`text-[1em] font-bold mb-[2%] ${viewMore ? "" : "line-clamp-1"}`}>{item?.title}</h3>
                <p className={`text-[0.95em] ${viewMore ? "" : "line-clamp-3"}`}> 
                    <span>{item?.description?.substring(0, viewMore ? item?.description?.length : 200)}</span>
                </p>
            ...
            <div className='mt-[1.5%] flex justify-between'>
                <div className='flex items-center w-[80%]'>
                    ...
                    <p className='text-[0.85em] flex flex-col mx-[2%]'>
                        <span>{item?.name}</span>
                        <span className={` ${viewMore ? "" : "line-clamp-2"}`}>{item?.designation}</span>
                    </p>
                </div>
