...
export default function Blog({h3Heading, text, card_bg}) {
      ...
      <section className="relative md:py-24 py-16" id="blog">
        <div className="container">
          <div className="grid grid-cols-1 pb-8 text-center">
            ...
            <h3 className="mb-4 md:text-2xl text-xl font-medium dark:text-white">{h3Heading}</h3>

            <p className="text-slate-500 dark:text-slate-300 max-w-xl mx-auto">{text}</p>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 gap-6 mt-8">
            {blogData.map((item, key) => (
              <div key={key} className={`blog relative rounded-md shadow shadow-slate-200 dark:shadow-slate-800 overflow-hidden ${card_bg}`}>
